                    ...
                    <td style="padding:8px 32px 0; text-align:{{ $isArabic ? 'right' : 'left' }};">
                        @if ($isArabic)
                            ...
                            <p style="font-size:14px; line-height:1.6; color:#475569; margin:0 0 24px;">
                                شكرًا لتسجيلكم في Atlasoft Syndic. اضغطوا على الزر أدناه لتفعيل حسابكم والبدء في تدبير نقابة سكناكم.
                            </p>
                        @else
                            <h1 style="font-size:18px; color:#0f172a; margin:0 0 16px;">Bonjour {{ $name }},</h1>
                            <p style="font-size:14px; line-height:1.6; color:#475569; margin:0 0 24px;">
                                Merci de vous être inscrit sur Atlasoft Syndic. Cliquez sur le bouton ci-dessous pour activer votre compte et commencer à gérer votre résidence.
                            </p>
                        @endif
                    </td>
                </tr>
                <tr>
                    <td align="center" style="padding:0 32px 24px;">
                        <a href="{{ $url }}" style="display:inline-block; background-color:#0891b2; color:#ffffff; font-size:14px; font-weight:bold; text-decoration:none; padding:12px 28px; border-radius:8px;">
                            {{ $isArabic ? 'تفعيل حسابي' : 'Vérifier mon email' }}
                        </a>
                    </td>
                </tr>
                <tr>
                    <td style="padding:0 32px 32px; text-align:{{ $isArabic ? 'right' : 'left' }};">
                        <p style="font-size:12px; line-height:1.6; color:#94a3b8; margin:0;">
                            @if ($isArabic)
                                هذا الرابط صالح لمدة 60 دقيقة. إذا لم تكونوا أنتم من قام بهذا التسجيل، يمكنكم تجاهل هذه الرسالة.
                            @else
                                Ce lien expire dans 60 minutes. Si vous n'êtes pas à l'origine de cette inscription, vous pouvez ignorer cet email.
                            @endif
                        </p>
                    </td>
                </tr>
