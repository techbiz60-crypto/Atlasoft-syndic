...
import type { RevenueCategory } from '../types/resources';
import { PageHeader } from '../components/PageHeader';
import { Field, Input } from '../components/ui/Input';
import { Button } from '../components/ui/Button';
import { ErrorAlert } from '../components/ui/Alert';

export function RevenueCategoriesPage() {
  const { user } = useAuth();
  ...
  async function loadCategories() {
    setIsLoading(true);
    try {
      const { data } = await api.get<{ data: RevenueCategory[] }>('/api/revenue-categories');
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      if (editingId) {
        await api.put(`/api/revenue-categories/${editingId}`, { name });
      } else {
        await api.post('/api/revenue-categories', { name });
      }
  ...
  async function handleDelete(id: number) {
    if (!confirm(t('revenueCategories.confirmDelete'))) {
      return;
    }

    try {
      await api.delete(`/api/revenue-categories/${id}`);
      await loadCategories();
    } catch (err) {
      setError(extractErrorMessage(err));
    }
  ...
  return (
    <div>
      <PageHeader title={t('revenueCategories.title')} subtitle={t('revenueCategories.subtitle')} />
        ...
        {isAdmin && (
          ...
          >
            <p className="text-sm font-semibold text-slate-900">
              {editingId ? t('revenueCategories.editTitle') : t('revenueCategories.addTitle')}
            </p>
              ...
              <table className="w-full text-start text-sm">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                    <th className="px-5 py-3">{t('revenueCategories.colName')}</th>
                    <th className="px-5 py-3">{t('revenueCategories.colRevenuesCount')}</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {categories.map((category) => (
                    <tr key={category.id} className="hover:bg-slate-50/60">
                      ...
                      <td className="px-5 py-3.5 text-slate-600">{category.revenues_count ?? 0}</td>
                      {isAdmin && (
...
function EmptyState() {
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white px-6 py-14 text-center">
      ...
      <p className="text-sm font-medium text-slate-900">{t('revenueCategories.emptyTitle')}</p>
      <p className="mt-1 text-sm text-slate-500">{t('revenueCategories.emptyDesc')}</p>
    </div>
  );
}
